    ...
    public class CommunicationInterface : IDisposable {
        ...
        void com_SerialDataEvent(object sender, SerialDataEventArgs e) {            
            if (isReceivingPacket == false) {                
                ...
                if (e.Data == 0xF0 && packetBuffer.Count == 1) {
                    System.Diagnostics.Debug.WriteLine("Second byte is also start byte.");
                } else {
                    packetBuffer.Add(e.Data);
                }
        ...
        public void SetFloatArray(byte id, float[] val) {
            byte[] arr_b = new byte [val.Length*sizeof(float)];
