...
function sendData(data) {
    ...
    XHR.addEventListener('load', function (event) {
        if (XHR.status != 200) {
            ...
            post_enabled = false;
            status_el.innerHTML = 'Error: Got invalid response with code ' + XHR.status + '.'
    ...
    XHR.addEventListener('error', function (event) {
        ...
        post_enabled = false;
        status_el.innerHTML = 'Error: Got error.'
        status_el.style.color = 'red';
    });

    XHR.addEventListener('timeout', function (event) {
        console.log('Server went away, disabling requests.');
        post_enabled = false;
        status_el.innerHTML = 'Server went away.'
        status_el.style.color = 'red';
    });
...
status_el.innerHTML = 'Initializing';
setTimeout(tick, TICK_INTERVAL);
