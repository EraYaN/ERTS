...
\documentclass{report}";
        ...
        static long hardwareSize = 0;
        static long softwareSize = 0;
        ...
        static void Main(string[] args)
        {
            ...
            Console.WriteLine("Hardware Code Size: {0}\r\nSoftware Code Size: {1}", hardwareSize, softwareSize);
            Console.ReadKey();
        }

        static bool IsIgnored(string fullname)
        {
		...
		static void processDirectory(DirectoryInfo d, int l)
		{
			...
			from fileQ in files
			where !IsIgnored(fileQ.FullName) && extensions.ContainsKey(fileQ.Extension)
			select fileQ;
			...
			if (fileQuery.Count() > 0)
			{
				...
				foreach (FileInfo fi in fileQuery)
				{
					...
					if (ext == ".vhdl" || ext == ".vhd")
					{
						...
						hardwareLines += lines;
                        hardwareSize += fi.Length;
						Console.WriteLine("Lines: {0} --> file: {1}", lines, fi.FullName, fi.Length);
					}
					else
					{
						long lines = CountLinesInFile(fi.FullName);
						softwareLines += lines;
                        softwareSize += fi.Length;
                        Console.WriteLine("Lines: {0}; Size: {2} --> file: {1}", lines, fi.FullName, fi.Length);
					}
				}
			}
