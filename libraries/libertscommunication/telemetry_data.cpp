...
TelemetryData::TelemetryData(uint16_t battery_voltage, int16_t phi, int16_t theta,
    ...
    _batteryVoltage = battery_voltage;
    _flightMode = flight_mode;
    _phi = phi;
    _theta = theta;
    _p = p;
    _q = q;
...
bool TelemetryData::is_valid() {
    return _flightMode != None;
}

void TelemetryData::to_buffer(uint8_t *buffer) {
    *(reinterpret_cast<uint16_t*>(&buffer[0])) = static_cast<uint16_t>(((_batteryVoltage & 0x0FFFu) << 4) | (_flightMode & 0x000Fu));
    ...
    *(reinterpret_cast<uint16_t*>(&buffer[12])) = _loopTime;
}
